...
public:
    ...
    void write(int fieldIndex, int value)
    {
        ...
        unique_lock<shared_mutex> lk(shared_mtx);
        if (fieldIndex == 0) {
            field0 = value;
        }
...
void setup_files(long num_commands)
{
    cout << "Генерація файлів" << endl;

    
    generate_command_file("var17.txt", { 5.0, 5.0, 30.0, 5.0, 55.0 }, num_commands);
    generate_command_file("equal.txt", { 20.0, 20.0, 20.0, 20.0, 20.0 }, num_commands);
    generate_command_file("custom.txt", { 5.0, 45.0, 5.0, 45.0, 0.0 }, num_commands);
...
bool load_commands(const string& filename, vector<string>& commands, long num_commands)
{
...
double run_test(const string& filename, int num_threads, long num_commands)
{

    vector<string> all_commands;

    
    if (!load_commands(filename, all_commands, num_commands)) {
        return -1.0;
    }
...
int main()
{
    ...
    for (size_t i = 0; i < files.size(); ++i) {
        ...
        for (size_t j = 0; j < thread_counts.size(); ++j) {
            ...
            double time_ms = run_test(files[i], thread_counts[j], NUM_COMMANDS); 
            results[i][j] = time_ms;
